...
class Mcp2210Commands(IntEnum):
    ...
    SET_NVRAM_PARAMETERS = 0x60
    GET_NVRAM_PARAMETERS = 0x61

    SEND_ACCESS_PASSWORD = 0x70

    GET_STATUS = 0x10

class Mcp2210NVRAMTarget(IntEnum):
    """
       Second byte of *_NVRAM_PARAMETERS
    """
    SPI_POWERUP_TRANSFER_SETTINGS = 0x10
    POWERUP_CHIP_SETTINGS = 0x20
    USB_KEY_PARAMETERS = 0x30
    USB_PRODUCT_NAME = 0x40
    USB_MANUFACTURER_NAME = 0x50
...
class Mcp2210USBKeyParameters(ValidatedDataClass):

    vid: int = 0
    pid: int = 0
    power_option_host_powered: bool = True
    power_option_self_powered: bool = False
    power_option_remote_wakeup: bool = False
    current: int = 0

    _structure_get = "<9xHH13xBB"
    _structure_set = "<HHBB"

    def _validate(self):
        pass

    def pack_get(self) -> bytes:
        return self._pack(Mcp2210USBKeyParameters._structure_get)

    def pack_set(self) -> bytes:
        return self._pack(Mcp2210USBKeyParameters._structure_set)


    def _pack(self, structure) -> bytes:
        power_option = (int(self.power_option_host_powered) << 7) | \
                       (int(self.power_option_self_powered) << 6) | \
                       (int(self.power_option_remote_wakeup) << 5)
    ...
    def unpack_from_get(self, data: bytes):
        self._unpack_from(Mcp2210USBKeyParameters._structure_get, data)

    def unpack_from_set(self, data: bytes):
        self._unpack_from(Mcp2210USBKeyParameters._structure_set, data)
    
    def _unpack_from(self, structure, data: bytes):
        """
        Unpacks the configuration from the binary format generated by the device.
        ...
        self.vid = int(unpacked[0])
        self.pid = int(unpacked[1])
        power_option = unpacked[2]
        self.power_option_host_powered  = (power_option & (1<<7)) != 0
        self.power_option_self_powered  = (power_option & (1<<6)) != 0
        self.power_option_remote_wakeup = (power_option & (1<<5)) != 0
        self.current = int(unpacked[3]) * 2
        print(unpacked)

    def __repr__(self):
        repr = 'vendor id:\t{0:#06X}\nproduct id:\t{1:#06X}\nhost powered:\t{2}\nself powered:\t{3}\nremote wakeup:\t{4}\ncurrent:\t{5} mA'.format(self.vid, self.pid, self.power_option_host_powered, self.power_option_self_powered, self.power_option_remote_wakeup, self.current)
        return repr

    def _validate(self):
        #assert self.power_option_host_powered ^ self.power_option_self_powered
        assert self.current <= 500
...
class Mcp2210(object):
    ...
    def __init__(self, serial_number: str, vendor_id: int = 0x04d8, product_id: int = 0x00de,
        ...
        self._hid = hid.Device(serial=serial_number, vid=vendor_id, pid=product_id)
        ...
        self._usb_key_parameter_needs_update = False
        ...
        self._get_usb_key_parameters()
        print(str(self._usb_key_parameter))
    ...
    def _get_usb_manufacturer_name(self):
        response = self._execute_command(bytes([Mcp2210Commands.GET_NVRAM_PARAMETERS, Mcp2210NVRAMTarget.USB_MANUFACTURER_NAME]))
    ...
    def _get_usb_key_parameters(self):
        response = self._execute_command(bytes([Mcp2210Commands.GET_NVRAM_PARAMETERS, Mcp2210NVRAMTarget.USB_KEY_PARAMETERS]))

        payload = bytes(response[3:31])
        self._usb_key_parameter.unpack_from_get(payload)

    def _set_usb_key_parameters(self):
        """
        
        """
        if self._usb_key_parameter_needs_update:
            request = [ Mcp2210Commands.SET_NVRAM_PARAMETERS, Mcp2210NVRAMTarget.USB_KEY_PARAMETERS, 0x00, 0x00]
            packed = self._usb_key_parameter.pack_set()

            print((bytes(request) + packed).hex())
